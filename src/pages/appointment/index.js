...
function CreateAppointment() {
    //STYLE//
    const customerPaperStyle = { padding: 20, height: '70vh', width: 600, margin: '40px auto', backgroundColor: '#eeeeee' }
    ...
    console.log('departments list!!', departments_list)
    const department_select_header = "Department"
    //EMPLOYEES CONST 
    const { get_employees_by_departmentid, employees_context } = useEmployeesContext();
...
///department///
    ...
    const childToParent = (childdata) => {
        setData(childdata);
        console.log("selected dp id : ",childdata);
    }

/////////
///employee///

const get_employees_data = async () => {
    await get_employees_by_departmentid(data)
  }
  useEffect(() => {
    get_employees_data(data)
  }, [])
                    ...
                    <Paper elevation={10} style={customerPaperStyle}>
                        ...
                        <Box sx={{ '& > :not(style)': { m: 1 } }}>
                            <form>
                                ...
                                <MultipleSelect childToParent={childToParent} list={departments_list} select_header={department_select_header} />
                                <br />
                                <MultipleSelect list={employees_list} select_header={department_select_header} />
                                <br />
                                <Button variant="contained" type="submit" value="Submit" fullWidth>Create</Button>
